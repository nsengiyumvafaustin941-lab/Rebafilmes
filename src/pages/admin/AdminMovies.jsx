...
import { ADMIN_SESSION_KEY, SETTINGS_KEY } from "../../utils/constants";
...
const VideoUploadZone = ({ onUrlReady }) => {
  ...
  const { state, progress, error, upload, cancel, reset } =
    useVideoUpload(onUrlReady);
    ...
    return (
      <div
        style={{
          ...
          padding: "1rem",
          background:
            state === "done" ? "rgba(34,197,94,.08)" : "rgba(59,130,246,.08)",
        }}
      >
        <div
          style={{
        ...
        >
          <span
            style={{
              fontSize: ".82rem",
              color: state === "done" ? "#22c55e" : "#93c5fd",
            }}
          >
            {state === "done"
              ? "✅ Upload complete — URL filled in below"
              : `Uploading… ${progress}%`}
          </span>
          {state === "uploading" ? (
            <button
  ...
  if (state === "error") {
    return (
      ...
      >
        <p style={{ color: "#f87171", fontSize: ".82rem", margin: 0 }}>
          ❌ {error}
        </p>
        <button
          type="button"
  ...
  return (
    ...
    >
      ...
      <p style={{ margin: ".3rem 0 0", fontSize: ".72rem", color: "#555" }}>
        MP4, MKV, WEBM, AVI, M3U8 — up to 5 GB
      </p>
      <input
        ref={fileInputRef}
        type="file"
  ...
  return (
    <>
      <button
        ...
        style={{
          background:
            uploadState === "done" ? "#22c55e" : "rgba(59,130,246,.15)",
...
const MovieModal = ({ initial, onSave, onClose, title }) => {
    ...
    setForm((p) => ({
      ...
      episodes: [
        ...(p.episodes || []),
        {
          ...
          s: 1,
          e: (p.episodes?.length || 0) + 1,
          title: `Episode ${(p.episodes?.length || 0) + 1}`,
          videoUrl: "",
        },
      ],
    }));
  ...
  const handleTMDBFetch = async () => {
    if (!form.title)
      return showToast("Please enter a title first to search", "error");

    const settings = JSON.parse(localStorage.getItem(SETTINGS_KEY)) || {};
    if (!settings.tmdbApiKey)
      return showToast("TMDB API Key missing! Add it in Settings.", "error");
    ...
    try {
      ...
      setForm((p) => ({
        ...
        poster: best.poster_path
          ? `${basePoster}${best.poster_path}`
          : p.poster,
        backdrop: best.backdrop_path
          ? `${baseImg}${best.backdrop_path}`
          : p.backdrop,
        year: best.release_date
          ? parseInt(best.release_date.substring(0, 4))
          : best.first_air_date
            ? parseInt(best.first_air_date.substring(0, 4))
            : p.year,
      ...
      showToast(
        "TMDB data fetched successfully! (incl. genre, country & trailer)",
      );
    } catch (err) {
        ...
        <div className="adm-form-grid">
          <div className="adm-form-group full">
            ...
            >
              ...
              >
                <Wand2 size={12} />{" "}
                {isFetching ? "Fetching..." : "Magic Fetch (TMDB)"}
              </button>
            </div>
            <input
              className="adm-input"
          ...
          <div className="adm-form-group full">
            <label className="adm-form-label">
              Trailer Key (YouTube Video ID / URL)
            </label>
            <input
              className="adm-input"
              value={form.trailerKey || ""}
              onChange={set("trailerKey")}
              placeholder="e.g. AyIZ9tiiN8I"
            />
          </div>
          {form.type === "movie" ? (
            <div className="adm-form-group full">
                  ...
                  style={{
                    ...
                    background:
                      videoTab === "upload"
                        ? "rgba(59,130,246,.3)"
                        : "transparent",
                    color: videoTab === "upload" ? "#93c5fd" : "#888",
                    cursor: "pointer",
                    transition: "all .15s",
                  }}
                >
              ...
              {videoTab === "paste" ? (
                <input
                  ...
                  placeholder="https://pub-xxxx.r2.dev/movies/movie.mp4"
                />
              ) : (
                <>
                  <VideoUploadZone
                ...
                <div className="adm-ep-list">
                  {(form.episodes || []).map((ep) => (
                    <div key={ep.id} className="adm-ep-row">
                      ...
                      <EpisodeUploadBtn
                        onUrlReady={(url) =>
                          handleUpdateEpisode(ep.id, "videoUrl", url)
                        }
                      />
                      <button
                        type="button"
                        className="adm-btn adm-btn-danger adm-btn-sm col-act"
          ...
          <div className="adm-form-group full">
            ...
            <textarea
              ...
              placeholder="Inkuru ya filme…"
            />
          </div>

          <div className="adm-form-row full">
            <span className="adm-form-row-label">
          ...
          >
            ...
            <div className="adm-form-group">
              ...
              <input
                ...
                placeholder="movies, rwanda movies..."
              />
            </div>
          </div>
        </div>
          ...
          <button
            ...
            onClick={() => {
              ...
              if (finalForm.type === "series" && finalForm.episodes) {
                finalForm.episodes = [...finalForm.episodes].sort((a, b) =>
    ...
    >
      <div className="adm-modal" style={{ maxWidth: 440 }}>
        ...
        <p style={{ color: "#888", fontSize: ".85rem", marginBottom: "1rem" }}>
          TMDB ID {movie.id} — metadata comes from TMDB. You can only curate how
          it appears on RebaFilme.
        </p>
        <div className="adm-form-group">
          <label className="adm-form-label">Badge</label>
          <input
  ...
  >
    <div className="adm-modal" style={{ maxWidth: 400 }}>
      ...
      <p style={{ color: "#aaa", fontSize: ".9rem", marginBottom: "1.5rem" }}>
        ...
        <strong style={{ color: "#fff" }}>"{movie.title}"</strong>? This cannot
        be undone.
      </p>
      <div className="adm-form-actions">
  ...
  return (
    <AdminLayout>
      <div className="adm-page-header">
        <div>
          ...
          <p className="adm-page-subtitle">
            {loading
              ? "Loading TMDB…"
              : `${allMovies.length} titles · auto-synced from TMDB`}
          </p>
        </div>
        <div
          style={{
      ...
      <div className="adm-info-banner">
        <strong>TMDB is the main catalog.</strong> Watch uses YouTube trailers.
        Download redirects to videodownloader.site. Pin titles as{" "}
        <em>Featured</em> or <em>Popular</em> — custom uploads are optional
        extras only.
      </div>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((m) => (
                <tr key={m.id}>
                  ...
                  <td style={{ fontWeight: 600, color: "#fff", maxWidth: 200 }}>
                    {m.title}
                  </td>
                  <td>
                  ...
                  <td>
                    <span
                      className={`adm-badge ${m.source === "tmdb" ? "adm-badge-default" : "adm-badge-admin"}`}
                    >
